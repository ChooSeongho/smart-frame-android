...
//            if(searchedDevice.getBondState() != BluetoothDevice.BOND_BONDED){
//                Log.d("choo","find device");
//            }
//            Log.d("choo","BLU_name = "+searchedDevice.getName());
            Log.d("choo", "in onReceive" + action);
            // When discovery finds a device
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                // Get the BluetoothDevice object from the Intent
                BluetoothDevice device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
                // If it's already paired, skip it, because it's been listed already
                Log.d("choo", "in action found");
                ...
                Log.d("choo", "inFinished");
                if (mNewDevicesArrayAdapter.getCount() == 0) {
                    String noDevices = "none found";
                    mNewDevicesArrayAdapter.add(noDevices);
                    Log.d("choo", "not device");
                }
            }
        }
    };
